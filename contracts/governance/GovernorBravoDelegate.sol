 ...
 * TODO
 * 
 * - Change block counts for diff interval at Klaytn.
 * - Change variable names: COMP --> vMMN.
 * - Check total cap: 10,000,000.
 * 
 * References
 *
...
contract GovernorBravoDelegate is GovernorBravoDelegateStorageV2, GovernorBravoEvents {
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        ...
        require(comp.getPriorVotes(msg.sender, block.number - 1) > proposalThreshold || isWhitelisted(msg.sender), "GovernorBravo::propose: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) external {
        ...
        if(msg.sender != proposal.proposer) {
            ...
            if(isWhitelisted(proposal.proposer)) {
                require((comp.getPriorVotes(proposal.proposer, block.number - 1) < proposalThreshold) && msg.sender == whitelistGuardian, "GovernorBravo::cancel: whitelisted proposer");
            }
            else {
                require((comp.getPriorVotes(proposal.proposer, block.number - 1) < proposalThreshold), "GovernorBravo::cancel: proposer above threshold");
            }
        }
